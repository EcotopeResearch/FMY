...
def load_data(scen, var, model, lat_targets, lon_targets, stations, daily, interp, datapath, dl_dat ):
    ...
    if os.path.exists(datapath+fileName):
        ...
        filehandle = Dataset(datapath+fileName,'r',format="NETCDF4") 
    else:
        print('Loading... ' + fullfilename)
        # Get data from the MACA address           
        filehandle = Dataset(fullfilename,'r',format="NETCDF4")     
                    
        # If you want to store it locally here we write the file locally.
        if dl_dat:
            print('Writing File... ' + datapath+fileName)
            write_file(datapath+fileName, filehandle)
